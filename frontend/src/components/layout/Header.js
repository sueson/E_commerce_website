    ...
    <nav className="navbar row">
      <div className="col-12 col-md-3">
        <div className="navbar-brand">
          <Link to={'/'}>
            <img width="150px" alt="Mern_build_logo" src="/images/logo.png" />
          </Link>
        </div>
      </div>
          ...
          <Dropdown className="d-inline">
            <Dropdown.Toggle variant="default text-white pr-5" id='dropdown-basic'>
              <figure className="avatar avatar-nav">
                <Image width= '50px' src={user.avatar ?? './images/default_image.jpg'} />  { /* using null coalescing operator (user avatar is optional if user doesn't gave avatar the default image will show up..) */}
              </figure>
              <span>{user.name}</span>
            </Dropdown.Toggle>
            <Dropdown.Menu>
